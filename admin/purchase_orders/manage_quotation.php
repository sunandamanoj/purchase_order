		...
		<form action="" id="po-form">
			...
			<div class="row">
				<div class="col-md-12">
                 ...
                 </b>Fill this for Quotation only
						<div class="col-md-6">
							
							
							<label for="quotation_notes" class="control-label">Agreement Notes</label>
							<textarea name="quotation_notes" id="quotation_notes" cols="10" rows="4" class="summernote form-control"><?php echo isset($quotation_notes) ? $quotation_notes : '' ?></textarea>
							
						</div>
						<div class="col-md-6">
							<label for="for_what_quotation" class="control-label">For What Quotation</label>
							<textarea name="for_what_quotation" id="for_what_quotation" cols="10" rows="4" class="form-control rounded-0"><?php echo isset($for_what_quotation) ? $for_what_quotation : '' ?></textarea>
						</div>
						<p  class="col-10">  <br>
                <b><br> 
                
                
            </b>Fill This For Purchase Orders Only


						<div class="col-md-6">
							<label for="purchase_orders_notes" class="control-label">Purchase Orders Notes</label>
							<textarea name="purchase_orders_notes" id="purchase_orders_notes" cols="10" rows="4" class="form-control rounded-0"><?php echo isset($purchase_orders_notes) ? $purchase_orders_notes : '' ?></textarea>
						</div>
						</div>
						
							<label for="status" class="control-label">Status</label>
							<select name="status" id="status" class="form-control form-control-sm rounded-0">
								<option value="3" <?php echo isset($status) && $status == 3 ? 'selected': '' ?>>Quotation</option>
								<option value="4" <?php echo isset($status) && $status == 4 ? 'selected': '' ?>>Purchase orders</option>
...
<script>
    ...
    $('.po-item').each(function(){
        ...
        $(this).find('.total-price').text(parseFloat(row_total).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}))
    })
    ...
    $('[name="discount_amount"]').val(parseFloat(discount_amount).toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 2}))
    var tax_perc = 0
    if($('[name="tax_percentage"]').val() > 0){
        tax_perc = $('[name="tax_percentage"]').val()
    }
    var tax_amount = _total * (tax_perc/100);
    $('[name="tax_amount"]').val(parseFloat(tax_amount).toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 2}))
    $('#sub_total').text(parseFloat(_total).toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 2}))
    $('#total').text(parseFloat(_total-discount_amount).toLocaleString("en-US", {minimumFractionDigits: 2, maximumFractionDigits: 2}))
}
			...
			$.ajax({
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						location.href = "./?page=purchase_orders/view_poo&id="+resp.id;
